...
async function triggerBroadcast(slot, input) {
  ...
  try {
    result.play = await ncm.resolvePlayableSongs(result.play || [], 11);
  } catch (err) {
...
export function start() {
  ...
  jobs.push(cron.schedule('0 7 * * *', () => {
    triggerBroadcast('morning', '早上好！今天天气怎么样？推荐一些适合早晨的音乐，帮我开启美好的一天。')
      .catch(err => console.error('[scheduler] Morning broadcast failed:', err));
  }, { timezone: 'Asia/Shanghai' }));

  // Noon: 12:00
  jobs.push(cron.schedule('0 12 * * *', () => {
    triggerBroadcast('noon', '中午了，推荐一些轻松的午餐音乐。')
      .catch(err => console.error('[scheduler] Noon broadcast failed:', err));
  }, { timezone: 'Asia/Shanghai' }));

  // Evening: 19:00
  jobs.push(cron.schedule('0 19 * * *', () => {
    triggerBroadcast('evening', '晚上好，推荐一些放松的音乐，帮我从忙碌中切换出来。')
      .catch(err => console.error('[scheduler] Evening broadcast failed:', err));
  }, { timezone: 'Asia/Shanghai' }));
