...
function getEnvironmentContext() {
  const now = new Date();
  const beijingHourStr = new Intl.DateTimeFormat('zh-CN', {
    timeZone: LOCAL_TIME_ZONE,
    hour: 'numeric',
    hour12: false,
  }).format(now);
  const hours = parseInt(beijingHourStr, 10);
  let timeOfDay = '深夜';
...
function formatImportedPlaylists() {
  const raw = readProjectFile('user/netease-playlists.json') || readProjectFile('user/playlists.json');
  if (!raw) return '';
  ...
  try {
    ...
    return playlists.slice(0, 12).map(playlist => {
      const tracks = (playlist.tracks || []).slice(0, 20).map(song =>
        `${song.artist || '未知歌手'} - ${song.name || song.song_name || '未知歌曲'}`
      );
      return `歌单: ${playlist.name} (${playlist.trackCount || tracks.length} 首)\n${tracks.join('\n')}`;
    }).join('\n\n');
  } catch {
    return '';
  }
...
export async function assemble({ input, db: dbModule, ncmResults, trigger }) {
  ...
  const playlists = formatImportedPlaylists();
  if (playlists) fragments.push(`[USER PLAYLISTS]\n${playlists}`);
